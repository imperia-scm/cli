      ...
      return getSelectedServices(nextState).length > 0
        ...
        : {
            state: cloneSelectionState(nextState, {
              validationMessage: 'No services selected. Select at least one service before running.',
            }),
            action: null,
          };
    case 'cancel':
...
function renderControlsBlock() {
  return [
    ...
    `  ${renderControlKey('g')} visible sync repos   ${renderControlKey('enter')} run   ${renderControlKey('esc')} ${renderControlKey('q')} cancel`,
  ];
}
...
export function renderRunSelectionScreen(state) {
  ...
  return [
    ...
    ...renderControlsBlock(),
    '',
    `${colors.bold('Services')} ${colors.dim('(select the services to launch with VS Code tasks)')}`,
    ...serviceLines,
    '',
...
export function buildNonInteractiveRunError() {
  return new Error('[run] Interactive terminal required. Use the dedicated VS Code service tasks in non-TTY sessions.');
}
...
export async function showRunSelectionPrompt(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const render = () => {
      ...
      stdout.write(renderRunSelectionScreen(state));
    };
