...
export function buildUsageText(commandDefinitions) {
  ...
  const commandLines = entries.map((entry) => `  ${entry.name.padEnd(maxNameLength, ' ')}  ${entry.description}`);

  return [
    'imperia-cli',
    '',
    'Run this workflow from VS Code tasks.',
    'Use `imp init` once to scaffold the workspace files.',
    'Use Ctrl+Shift+B or run a task from .vscode/tasks.json.',
    '',
    'Commands:',
    ...commandLines,
  ].join('\n');
