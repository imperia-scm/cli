...
export async function invokeTrackedBuild({ targetRepoRoot, stateFileName, solutionPath, taskLabel, interactive = true }) {
  ...
  if (previousFingerprint === fingerprint) {
    log(taskLabel, 'SKIP', `${taskLabel}: No git changes since last successful build.`);
    return;
  }
  ...
  if (result.code !== 0) {
    if (buildContainsOnlyTestFailures(result.outputText)) {
      ...
      log(taskLabel, 'OK', `${taskLabel}: Build failure only in test projects. Continuing startup.`);
      return;
    }
...
export async function syncRepository(targetRepoRoot, repoLabel, options = {}) {
  const interactive = options.interactive ?? true;

  await runCommand({
    scope: 'sync-repository',
  ...
  if (local === remote) {
    log('sync-repository', 'OK', `${repoLabel}: already up to date.`);
    return;
  }

  log('sync-repository', 'START', `${repoLabel}: changes detected, pulling.`);
  await runCommand({
    scope: 'sync-repository',
